( function ( global ) {
	...
	var wellComComponents = {
		cardInline: {
			methods: {
				wellComCardInlineToggle: function ( cards, card, activeName ) {
					...
					for ( var i = 0, len = cards.length; i < len; ++i )
					{
						if ( cards[ i ] === card )
						{
							cards[ i ][ activeName ] = true;
						}
						else
						{
							cards[ i ][ activeName ] = false;
						}
					}
				}
			}
			...
			methods: { /* menu - methods [ start ] */
				wellComMenuToggle: function ( menu, showName ) {
					...
					if ( wellComComponentsStore.menu.menus[ uid ] === undefined )
					{
						...
						wellComComponentsStore.menu.menus[ uid ].push( {
							showName: showName,
							menu: menu
						} );
					}
			...
			methods: { /* contextMenu - methods [ start ] */
				wellComContextMenuShow: function ( menu, evt, nameConfig ) {
					...
					if ( wellComComponentsStore.contextMenu.menus[ uid ] === undefined )
					{
						...
						wellComComponentsStore.contextMenu.menus[ uid ].push( {
							showName: showName,
							menu: menu
						} );
					}
				...
				wellComContextMenuHideAll: function () {
					var key,
					    i,
						len,
						m;
	...
	var wellA = {
		...
		computed: {
			aCls: function () {
				return {
					"well-com-a": true,
					"well-com-a--small": this.size === "small",
		...
		computed: {
			btnCls: function () {
				return {
					...
					"well-com-btn--disabled": this.state === "disabled",
				...
				<div class="well-base-wrapper well-ori-margin--0">\
					...
					<div class="well-ori-display__flex-row--sb well-ori-padding--normal well-com-card__des-wrapper">\
						<div class="well-com-card__des">\
							<slot name="des-1"></slot>\
						</div>\
						<div class="well-com-card__des">\
							<slot name="des-2"></slot>\
						</div>\
					</div>\
				</div>\
		...
		computed: {
				...
				switch ( this.mode )
				{
					case "default":
						return {
							...
							"well-com-card-inline--active": this.active
						};
			...
			cardInlineTitleCls: function () {
				switch ( this.mode )
				{
					case "default":
						return {
							"well-ori-width--15": !this.mini,
							"well-ori-width--100": this.mini,
							"well-com-card-inline__title": !this.mini
						};
					...
					<div :class="cardInlineDes1Cls">\
						...
						<template v-else>\
							<div class="well-com-card-inline__des-1-primary">\
								<slot name="des-1-primary"></slot>\
							</div>\
							<div class="well-com-card-inline__des-1-secondary">\
								<slot name="des-1-secondary"></slot>\
							</div>\
						</template>\
					</div>\
	...
	var wellCarouselDefault = {
		name: "well-carousel-default",
	...
	Vue.component( "well-carousel-default", wellCarouselDefault );
		...
		template: '\
			...
			>\
				<template #first-item="firstItemProps">\
					<slot name="first-item" :firstItem="firstItemProps.firstItem"></slot>\
				</template>\
				<template #item="itemProps">\
					<slot name="item" :item="itemProps.item"></slot>\
				</template>\
				<template #last-item="lastItemProps">\
					<slot name="last-item" :lastItem="lastItemProps.lastItem"></slot>\
				</template>\
			</well-carousel-default>\
			<well-carousel-mobile\
				v-else-if="wellComData.isMobile && mode === \'default\'"\
				:items="items"\
				:auto="auto"\
				:delay="delay"\
				:height="height"\
			...
			<well-carousel-touch\
				v-else-if="wellComData.isMobile && mode === \'touch\'"\
				:items="items"\
				:auto="auto"\
				:delay="delay"\
				:height="height"\
	...
	var wellCounter = {
		...
		computed: {
			counterCls: function () {
				return {
					...
					"well-com-counter--dot": this.count === undefined
				};
			},
		...
		template: '\
			<div :class="counterCls">\
				{{ valueOutput }}<span v-if="this.count > this.maxCount || -this.count > this.maxCount"\
	...
	var wellFile = {
		...
		computed: {
			...
			iconCls: function () {
				return {
					...
					"fa-times-circle": this.fileTypeInvalid || this.fileSizeInvalid || this.fileTotalSizeInvalid || this.uploadFail
				};
			}
		},
		...
		created: function () {
			...
			this.fileSizeNeedToValidate = typeof this.singleMaxSize === "number" && this.singleMaxSize > 0;
		},
		...
		methods: {
			changeHandler: function ( evt ) {
				...
				this.fileTotalSizeInvalid = false;
				...
				else
				{
					...
					for ( i = 0; i < len; ++i )
					{
						...
						if ( this.fileSizeNeedToValidate && !this.wellComFileSizeValidator( this.singleMaxSize, files[ i ].size ) )
						{
					...
					if ( this.fileSizeNeedToValidate && !this.wellComFileSizeValidator( this.totalMaxSize, totalSize ) )
					{
						this.fileTotalSizeInvalid = true;
						return;
					}
			...
			<div class="well-com-file">\
				...
				<label\
					...
					<span class="well-com-file__label-txt">\
						...
						<template v-else-if="!uploadStart && fileSelected && !multiple && fileSizeInvalid">\
							<slot name="selected-err-size-txt">所选文件的大小超过指定大小</slot>\
							<span class="well-com-file__label-reselect">重新选择</span>\
						</template>\
						<template v-else-if="!uploadStart && fileSelected && multiple && fileSizeInvalid">\
							<slot name="selected-err-size-txt">所选文件中有部分文件的大小超过指定大小</slot>\
							<span class="well-com-file__label-reselect">重新选择</span>\
						</template>\
						<template v-else-if="!uploadStart && fileSelected && fileTotalSizeInvalid">\
							<slot name="selected-err-total-size-txt">所选文件的总大小超过指定大小</slot>\
							<span class="well-com-file__label-reselect">重新选择</span>\
						</template>\
	...
	var wellInfiniteScroll = {
		...
		methods: {
			scrollHandler: function () {
				...
				if ( wrapperHeight - ( this.containerHeight + containerScrollTop ) <= this.distance )
				{
					this.$emit( "request" );
				}
			}
		...
		template: '\
			<div :class="infiniteScrollCls"\
				...
				<div v-if="noMoreItems"\
					 class="well-com-infinite-scroll__no-more-items-txt">\
					<slot name="no-more-items-txt">已滚动到最底部</slot>\
				</div>\
			</div>\
		'
	};
	...
	var wellModal = {
				...
				switch ( this.auto )
				{
					case false:
						return {
							...
							"well-com-modal--extra-large": this.size === "extra-large"
						};
		...
		methods: {
			...
			confirm: function () {
				...
				this.operation().then( function ( data ) {
			...
			<div :class="paginationCls">\
				<template v-if="pages === 0">\
					<span :class="prevBtnCls">\
						<i class="well-com-pagination__icon well-com-pagination__prev-btn-icon fa fas fa-angle-left"></i>\
					...
					<span :class="nextBtnCls">\
						...
						<i class="well-com-pagination__icon well-com-pagination__next-btn-icon fa fas fa-angle-right"></i>\
					</span>\
				</template>\
				<template v-else>\
					...
					<span v-if="pages > 9 && ( ( requestingPage > 5 && requestingPage < ( pages - 4 ) ) || requestingPage >= ( pages - 4 ) )">\
						<i class="well-com-pagination__icon well-com-pagination__icon-ellipsis fa fas fa-ellipsis-h"></i>\
					</span>\
	...
	var wellSidebar = {
		...
		methods: {
			...
			touchEndHandler: function ( evt ) {
				...
				if ( this.touchMoveDis >= 50 )
				{
					this.open = true;
				}
				else if ( -this.touchMoveDis >= 50 )
				{
					this.open = false;
				}
			},
	...
	Vue.component( "well-tab", wellTab );
